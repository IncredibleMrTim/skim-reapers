...
Dev and production are separate Sanity datasets (`development` and `production`) with independent content — see [`deploy-krystal.md`](deploy-krystal.md).
...
## Studio navigation (`structure.ts`)

`src/sanity/structure.ts` passes a custom `StructureResolver` to `structureTool`, which **replaces** Studio's default document list entirely — it doesn't layer on top of it. Singleton document types (`homePage`, `aboutPage`, ...) are registered in `schemaTypes/index.ts`, but that alone only makes the type valid and queryable; it does **not** put it anywhere in the Studio nav. Every singleton needs its own explicit list item in `structure.ts`, or it exists as a schema with no way to open it in the editor:
